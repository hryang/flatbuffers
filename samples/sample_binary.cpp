...
int main(int /*argc*/, const char * /*argv*/[]) {
  ...
  flatbuffers::Offset<Monster> mloc = CreateMonster(builder, &vec, 150, 80, name, inventory,
                            Color_Blue);
  ...
  const MyGame::Sample::Monster * monster = GetMonster(builder.GetBufferPointer());
  ...
  const flatbuffers::Vector<uint8_t> * inv = monster->inventory();
  assert(inv);
  assert(inv->Get(9) == 9);
  (void)inv;
}

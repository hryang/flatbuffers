...
void GenComment(const std::vector<std::string> &dc, std::string *code_ptr,
  ...
  if (config != NULL && config->first_line != NULL) {
    code += std::string(prefix) + std::string(config->first_line) + "\n";
  }
  std::string line_prefix = std::string(prefix) +
      ((config != NULL && config->content_line_prefix != NULL) ?
  ...
  if (config != NULL && config->last_line != NULL) {
...
static void GenStruct(const LanguageParameters &lang, const Parser &parser,
  ...
  for (std::vector<FieldDef*>::const_iterator it = struct_def.fields.vec.begin();
    ...
    if (opts.mutable_buffer) {
      Type underlying_type = field.value.type.base_type == BASE_TYPE_VECTOR
                    ? field.value.type.VectorType()
                    : field.value.type;
      // boolean parameters have to be explicitly converted to byte representation
        std::string setter_parameter = underlying_type.base_type == BASE_TYPE_BOOL ? "(byte)(" + field.name + " ? 1 : 0)" : field.name;
      ...
      //a vector mutator also needs the index of the vector element it should mutate
        std::string mutator_params = (field.value.type.base_type == BASE_TYPE_VECTOR ? "(int j, " : "(") +
                            GenTypeNameDest(lang, underlying_type) + " " + 
                            field.name + ") { ";
        std::string setter_index = field.value.type.base_type == BASE_TYPE_VECTOR
...
static bool SaveClass(const LanguageParameters &lang, const Parser &parser,
  ...
  if (!namespace_general.empty()) code += lang.namespace_end;
    std::string filename = namespace_dir + defname + lang.file_extension;
...
std::string BinaryFileName(const Parser &parser,
                           const std::string &path,
                           const std::string &file_name) {
    std::string ext = parser.file_extension_.length() ? parser.file_extension_ : "bin";
...
std::string BinaryMakeRule(const Parser &parser,
  ...
  std::string make_rule = BinaryFileName(parser, path, filebase) + ": " +
      file_name;
  std::set<std::string> included_files = parser.GetIncludedFilesRecursive(

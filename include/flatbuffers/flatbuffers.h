...
private:
  template<typename K> static int KeyCompare(const void *ap, const void *bp) {
    ...
    typename IndirectHelper<T>::return_type table = IndirectHelper<T>::Read(data, 0);
 ...
 public:
  ...
  uint8_t *make_space(size_t len) {
    if (len > static_cast<size_t>(cur_ - buf_)) {
      ...
      size_t largest_align = AlignOf<largest_scalar_t>();
...
private:
   ...
   struct LessOffset
    {
        explicit LessOffset(FlatBufferBuilder* builder): mBuilder(builder) {}
        
        bool operator()(const Offset<T>& a, const Offset<T>& b) const
        {
            T* table_a = reinterpret_cast<T *>(mBuilder->buf_.data_at(a.o));
            T* table_b = reinterpret_cast<T *>(mBuilder->buf_.data_at(b.o));
...
public:
  template<typename T> Offset<Vector<Offset<T> > > CreateVectorOfSortedTables(
      ...
      std::sort(v, v + len, LessOffset<T>(this));
...
class Table {
 public:
  ...
  template<typename P> P GetStruct(voffset_t field) const {
    ...
    return field_offset ? reinterpret_cast<P>(p) : NULL;
  }
  ...
  uint8_t *GetAddressOf(voffset_t field) {
    voffset_t field_offset = GetOptionalFieldOffset(field);
    return field_offset ? data_ + field_offset : NULL;
  }
